...
CURR_SSID=$(iwctl station wlan0 show | awk "NR == 7" | sed -e 's/\<Connected network\>//g' | tr -d ' ')
...
CHENTRY=$(echo -e "$MENU" | uniq -u | rofi -dmenu -p "WiFi SSID" -lines "$LINE_COUNT" -a "$HIGHLINE" -location "$POSITION" -yoffset "$Y_OFF" -xoffset "$X_OFF" -font "$FONT" -width -"$R_WIDTH" -theme /home/kumao/.config/rofi/launchers/style.rasi)
CHSSID=$(echo "$CHENTRY" | sed  's/\s\{2,\}/\|/g' | awk -F "|" '{print $1}')

## Support manual SSID entry.
if [ "$CHENTRY" = "manually connect to a network" ] ; then
	MSSID=$(echo "Enter your network's SSID." | rofi -dmenu -p "SSID: " -font "$FONT" -lines 1 -theme /home/kumao/.config/rofi/launchers/style.rasi)
	WIFI_PASS=$(echo "Enter the network password." | rofi -dmenu -password -p "Password: " -lines 1 -location "$POSITION" -yoffset "$Y_OFF" -xoffset "$X_OFF" -font "$FONT" -width -"$R_WIDTH" -theme /home/kumao/.config/rofi/launchers/style.rasi)
